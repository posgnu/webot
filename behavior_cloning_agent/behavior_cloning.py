...
class BehaviorCloning:
    ...
    def build_model(self):
        ...
        self.m = Net(self.vocab, self.dev)
    ...
    def build_vocab(self):
        ...
        # To numpy arrays
        states, _ = map(np.array, zip(*data))
        utterance_states = np.array(list(map(lambda x: x["utterance"], states)))
        utterances = [x.replace(" ", "") for x in utterance_states]
    ...
    def play(self, episodes: int = 10):
        ...
        for _ in range(episodes):
            ...
            while not done:
                ...
                # FIXME: state dtype should be float32. otherwise self.preprocess output different result
                img_state = np.moveaxis(
                    np.array(img_states[0]), 2, 0
                )  # channel first image
                img_state = self.img_preprocess(img_state)

                goal_state = states[0].utterance.replace(" ", "")
